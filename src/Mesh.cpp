...
bool Mesh::Load(const std::string &fileName, Renderer *renderer) {
  ...
  if (!LevelLoader::LoadJSON(fileName, doc)) {
    KE_CORE_ERROR("Failed to load mesh %s", fileName.c_str());
    return false;
  }
  ...
  // Check the version
  if (ver != 1) {
    KE_CORE_ERROR("Mesh %s not version 1", fileName.c_str());
    return false;
  }
  ...
  if (!textures.IsArray() || textures.Size() < 1) {
    KE_CORE_ERROR("Mesh %s has no textures, there should be at least one",
                  fileName.c_str());
    return false;
  }
  ...
  if (!vertsJson.IsArray() || vertsJson.Size() < 1) {
    KE_CORE_ERROR("Mesh %s has no vertices", fileName.c_str());
    return false;
  }
  ...
  for (rapidjson::SizeType i = 0; i < vertsJson.Size(); i++) {
    ...
    if (!vert.IsArray()) {
      KE_CORE_ERROR("Unexpected vertex format for %s", fileName.c_str());
      return false;
    }
  ...
  if (!indJson.IsArray() || indJson.Size() < 1) {
    KE_CORE_ERROR("Mesh %s has no indices", fileName.c_str());
    return false;
  }
  ...
  for (rapidjson::SizeType i = 0; i < indJson.Size(); i++) {
    ...
    if (!ind.IsArray() || ind.Size() != 3) {
      KE_CORE_ERROR("Invalid indices for %s", fileName.c_str());
      return false;
    }
