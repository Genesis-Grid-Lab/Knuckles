...
namespace Knuckles {
UIScreen::UIScreen(Application *game)
    : mApplication(game), mTitle(nullptr), mBackground(nullptr),
      mTitlePos(0.0f, 300.0f), mNextButtonPos(0.0f, 200.0f),
      mBGPos(0.0f, 250.0f), mState(EActive) {
  // Add to UI Stack
  mApplication->PushUI(this);
  mFont = mApplication->GetFont("Assets/Carlito-Regular.ttf");
  mButtonOn =
      mApplication->GetRenderer()->GetTexture("Assets/ButtonYellow.png");
  mButtonOff = mApplication->GetRenderer()->GetTexture("Assets/ButtonBlue.png");
}
...
void UIScreen::ProcessInput(const struct InputState &state) {
  ...
  if (!mButtons.empty()) {
    ...
    mousePos.x -= mApplication->GetRenderer()->GetScreenWidth() * 0.5f;
    mousePos.y =
        mApplication->GetRenderer()->GetScreenHeight() * 0.5f - mousePos.y;
