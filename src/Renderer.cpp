...
namespace Knuckles {
Renderer::Renderer(Application *game)
    : mApplication(game), mSpriteShader(nullptr), mMeshShader(nullptr),
...
bool Renderer::Initialize(std::string title, float screenWidth,
                          float screenHeight) {
  ...
  mWindow =
      SDL_CreateWindow(title.c_str(), 100, 100, static_cast<int>(mScreenWidth),
                       static_cast<int>(mScreenHeight), SDL_WINDOW_OPENGL);
  if (!mWindow) {
    KE_CORE_ERROR("Failed to create window: %s", SDL_GetError());
    return false;
  }
  ...
  if (glewInit() != GLEW_OK) {
    KE_CORE_ERROR("Failed to initialize GLEW.");
    return false;
  }
  ...
  if (!LoadShaders()) {
    KE_CORE_ERROR("Failed to load shaders.");
    return false;
  }
  ...
  //{
  //	KE_CORE_ERROR("Failed to create render target for mirror.");
  //	return false;
  //}
  ...
  if (!mGBuffer->Create(width, height)) {
    KE_CORE_ERROR("Failed to create G-buffer.");
    return false;
  }
...
void Renderer::Draw() {
...
#ifdef KE_EDITOR
  Draw3DScene(GetFBO(), mView, mProjection, false);
#else
  ...
  // Draw any UI screens
  for (auto ui : mApplication->GetUIStack()) {
  ...
  // add rendrered texture to ImGui scene window
  uint64_t textureID = GetTextureId();
