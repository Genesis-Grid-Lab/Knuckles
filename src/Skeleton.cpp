...
bool Skeleton::Load(const std::string &fileName) {
  ...
  if (!LevelLoader::LoadJSON(fileName, doc)) {
    KE_CORE_ERROR("Failed to load skeleton %s", fileName.c_str());
    return false;
  }
  ...
  // Check the metadata
  if (ver != 1) {
    KE_CORE_ERROR("Skeleton %s unknown format", fileName.c_str());
    return false;
  }

  const rapidjson::Value &bonecount = doc["bonecount"];
  if (!bonecount.IsUint()) {
    KE_CORE_ERROR("Skeleton %s doesn't have a bone count.", fileName.c_str());
    return false;
  }
  ...
  if (count > MAX_SKELETON_BONES) {
    KE_CORE_ERROR("Skeleton %s exceeds maximum bone count.", fileName.c_str());
    return false;
  }
  ...
  if (!bones.IsArray()) {
    KE_CORE_ERROR("Skeleton %s doesn't have a bone array?", fileName.c_str());
    return false;
  }
