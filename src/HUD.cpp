...
namespace Knuckles
{
HUD::HUD(Application *game)
    : UIScreen(game), mRadarRange(2000.0f), mRadarRadius(92.0f),
      mTargetEnemy(false) {
  Renderer *r = mApplication->GetRenderer();
...
void HUD::Draw(Shader *shader) {
  ...
  // Texture* mirror = mApplication->GetRenderer()->GetMirrorTexture();
  // DrawTexture(shader, mirror, Vector2(-350.0f, -250.0f), 1.0f, true);
  // Texture* tex =
  // mApplication->GetRenderer()->GetGBuffer()->GetTexture(GBuffer::EDiffuse);
...
void HUD::UpdateCrosshair(float deltaTime) {
  ...
  Vector3 start, dir;
  mApplication->GetRenderer()->GetScreenDirection(start, dir);
  LineSegment l(start, start + dir * cAimDist);
  // Segment cast
  PhysWorld::CollisionInfo info;
  if (mApplication->GetPhysWorld()->SegmentCast(l, info)) {
    // Is this a target?
    for (auto tc : mTargetComps) {
      if (tc->GetOwner() == info.mObject) {
        mTargetEnemy = true;
        break;
      }
    }
...
void HUD::UpdateRadar(float deltaTime) {
  ...
  Vector3 playerPos = mApplication->GetPlayer()->GetPosition();
  Vector2 playerPos2D(playerPos.y, playerPos.x);
  // Ditto for player forward
  Vector3 playerForward = mApplication->GetPlayer()->GetForward();
  ...
  for (auto tc : mTargetComps) {
    ...
    Vector2 ObjectPos2D(targetPos.y, targetPos.x);

    // Calculate vector between player and target
    Vector2 playerToTarget = ObjectPos2D - playerPos2D;
