...
AudioSystem::AudioSystem(Application *game)
    : mApplication(game), mSystem(nullptr), mLowLevelSystem(nullptr) {}
...
bool AudioSystem::Initialize() {
  ...
  if (result != FMOD_OK) {
    KE_CORE_ERROR("Failed to create FMOD system: %s", FMOD_ErrorString(result));
    return false;
  }
  ...
  if (result != FMOD_OK) {
    KE_CORE_ERROR("Failed to initialize FMOD system: %s", FMOD_ErrorString(result));
    return false;
  }
