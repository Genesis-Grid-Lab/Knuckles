...
bool Shader::CompileShader(const std::string &fileName, GLenum shaderType,
  ...
  if (shaderFile.is_open()) {
    ...
    if (!IsCompiled(outShader)) {
      KE_CORE_ERROR("Failed to compile shader {0}", fileName.c_str());
      return false;
    }
  } else {
    KE_CORE_ERROR("Shader file not found: {0}", fileName.c_str());
    return false;
  }
...
bool Shader::IsCompiled(GLuint shader) {
  ...
  if (status != GL_TRUE) {
    ...
    KE_CORE_ERROR("GLSL Compile Failed:\n{0}", buffer);
    return false;
  }
...
bool Shader::IsValidProgram() {
  ...
  if (status != GL_TRUE) {
    ...
    KE_CORE_ERROR("GLSL Link Status:\n{0}", buffer);
    return false;
  }
